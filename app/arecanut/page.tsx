...
import { ChevronDown, X, CheckCircle, TrendingUp, Clock } from "lucide-react";
...
import { arecanutGuideData } from "@/lib/arecanut-guide-data";
import { useProducts } from "@/hooks/useProducts";
import { Product } from "@/types/product";
import { Button } from "@/components/ui/button";
import { InfiniteScrollTrigger } from "@/components/InfiniteScrollTrigger";

export default function ArecanutPage() {
  ...
  // Filter logic handled client-side for price ranges since API doesn't support it yet
  const filteredProducts = products
    ...
    .sort((a, b) => {
      // Sort logic for fields not handled by API sort parameter
      switch (sortBy) {
        case "price-low":
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="container px-4 pt-4 md:px-6">
        <PageHero
          title="Arecanut Plants Online"
          description="India's premier source for quality Arecanut plants. Explore our extensive collection of carefully cultivated varieties for your farm or garden."
          imageSrc="/arecanut-hero.png"
          imageAlt="Arecanut Plants"
        />
      </div>
      ...
      <section className="w-full pb-8 pt-0 bg-green-50 dark:bg-green-950">
        <div className="container px-4 md:px-6">
          ...
          <div className="mb-6 text-sm text-muted-foreground">
            Showing {filteredProducts.length} arecanut plants
            {activeFilterCount > 0 && (
              <>
                {" · Filters: "}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProducts.map((product) => (
              <ProductCard
                ...
                highlights={[]} // Simplified for dynamic data
          ...
          {!loading && products.length === 0 && (
            <div className="col-span-full text-center py-12">
              <p className="text-muted-foreground">
                No arecanut plants found matching your criteria
              </p>
            </div>
          )}
      ...
      <section className="w-full py-8 bg-green-50 dark:bg-green-950/30 border-t border-green-100 dark:border-green-900">
        <div className="container px-4 md:px-6">
          <h2 className="text-2xl md:text-3xl font-bold mb-6 text-center">
            {arecanutGuideData.title}
          </h2>
          <p className="text-muted-foreground text-center mb-8 max-w-3xl mx-auto">
            {arecanutGuideData.subtitle}
          </p>

          <div className="grid md:grid-cols-3 gap-6 mb-8">
            {arecanutGuideData.factors.map((factor) => {
              const Icon =
                factor.icon === "trending-up"
                  ? TrendingUp
                  : factor.icon === "clock"
                  ? Clock
                  : CheckCircle;
              return (
                <div
                  key={factor.id}
                  className="bg-green-50 dark:bg-green-950/30 rounded-lg p-6 border border-green-200 dark:border-green-800"
                >
                  <div className="flex items-start gap-3">
                    <div className="p-2 bg-green-600 text-white rounded-full flex-shrink-0 mt-1">
          ...
          {/* Need Help Section */}
          <div className="max-w-2xl mx-auto p-6 bg-green-50 dark:bg-green-950/30 rounded-lg border-2 border-green-200 dark:border-green-800 text-center">
            <h3 className="font-semibold mb-2">
              {arecanutGuideData.helpSection.title}
            </h3>
            <p className="text-sm text-muted-foreground mb-4">
              {arecanutGuideData.helpSection.description}
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a
                href={`https://wa.me/${config.whatsappNumber}?text=Hi, I need help choosing the right arecanut plant variety`}
                target="_blank"
                rel="noopener noreferrer"
              >
                <Button className="bg-green-700 hover:bg-green-800 text-white shadow-md px-6 font-bold">
                  {arecanutGuideData.helpSection.primaryCta.label}
                </Button>
              </a>
              <div className="text-sm font-medium text-muted-foreground">
                {arecanutGuideData.helpSection.secondaryContact.label}:{" "}
      ...
      {/* Promo Section - Using PageHero style */}
      <div className="container px-4 py-8 md:px-6">
        <PageHero
          title="Elevate your farming journey with ArecaMart!"
          description="ArecaMart is your one-stop shop for premium Arecanut plants online. We offer a diverse selection of healthy, top-quality plants, including high-yielding varieties and disease-resistant cultivars."
          imageSrc="/promo-arecanut.png"
          imageAlt="Arecanut Plants Promo"
        />
      </div>
      <ProductModal
        product={
